...
void addMutationProposal(map<int, MutationProposal>& mutationProposals, int startPos, int readPos, const string& readSeq, const string& reference, char mutationType, int count) {
...
void detectMutations(const vector<SamEntry>& samEntries, const string& reference, const string& outputCsv) {
    ...
    for (const auto& entry : samEntries) {
        ...
        for (const auto& op_count : cigarOperations) {
            ...
            addMutationProposal(mutationProposals, refPos, readPos, readSeq, reference, op, count);
    ...
    for (auto& proposal : mutationProposals) {
        ...
        string type;
        char base = ' ';
        if (votes.substitutionVotes >= votes.insertionVotes && votes.substitutionVotes >= votes.deletionVotes && votes.substitutionVotes >= votes.noneVotes) {
...
int main() {
    ...
    string outputCsv = "mutations_newest.csv";
