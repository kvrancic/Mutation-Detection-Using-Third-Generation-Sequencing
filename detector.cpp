...
void detectMutations(const vector<SamEntry>& samEntries, const string& reference, const string& outputCsv, const string& logFilename) {
    ...
    for (auto& proposal : mutationProposals) {
        ...
        if (votes.noneVotes >= votes.substitutionVotes - 1 && votes.noneVotes >= votes.insertionVotes - 1 && votes.noneVotes >= votes.deletionVotes - 1) {
        ...
        // Ispis rezultata
        if (base != ' ') {
            outfile  << (type == "insertion" ? "I" : (type == "deletion" ? "D" : "X")) << "," << pos << "," << base << "\n";
